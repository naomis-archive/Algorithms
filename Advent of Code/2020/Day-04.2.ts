const passportValidate = (list: string): number => {
  const bigArray = list.split(/(\n){2,}/g).filter((el) => el !== "\n");
  const splitValueArray = bigArray.map(el => el.split(/\s|\n/));
  let count = 0;
  for (const passport of splitValueArray) {
    let validProperties = 0;
    for (const field of passport) {
      const [key, value] = field.split(":");
      switch (key) {
        case "byr":
          if (parseInt(value) >= 1920 && parseInt(value) <= 2002) validProperties++
          break
        case "iyr":
          if (parseInt(value) >= 2010 && parseInt(value) <= 2020) validProperties++
          break
        case "eyr":
          if (parseInt(value) >= 2020 && parseInt(value) <= 2030) validProperties++
          break
        case "hgt":
          const measurement = parseInt(value.replace(/\D/g, ""));
          if (value.endsWith("cm") && measurement >= 150 && measurement <= 193) validProperties++
          if (value.endsWith("in") && measurement >= 59 && measurement <= 76) validProperties++
          break
        case "hcl":
          const checkHair = /^#[\da-f]{6}$/
          if (checkHair.test(value)) validProperties++
          break
        case "ecl":
          const checkEyes = /^(amb|blu|brn|gry|grn|hzl|oth)$/
          if (checkEyes.test(value)) validProperties++
          break
        case "pid":
          const checkPass = /^\d{9}$/
          if (checkPass.test(value)) validProperties++
          break
        default:
          break
      }
    }
    if (validProperties === 7) {
      count++
    }
  }
  return count;
