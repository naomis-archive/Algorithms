const passwordValidate = (text: string): number => {
    let validCount = 0;
    const passwordArray = text
      .split("\n")
      .map((el) => {
        const splitValue = el.split(": ");
        const splitNumbers = splitValue[0].split(" ")[0]
        const low = splitNumbers.split("-")[0]
        const high = splitNumbers.split("-")[1]
        const splitLetter = splitValue[0].split(" ")[1]
        return {
          password: splitValue[1],
          letter: splitLetter,
          position1: Number(low) - 1,
          position2: Number(high) - 1
        }
        })
    for (const passwordSet of passwordArray) {
      const {password, letter, position1, position2} = passwordSet;
      if (password[position1] === letter && password[position2] !== letter) {
        validCount++
      }
      if (password[position2] === letter && password[position1] !== letter) {
        validCount++
      }
    }
  ...
  const input = `9-11 p: pppppppppxblp
  2-4 b: bbxbb
  3-5 q: dqfqb
  5-8 g: ggcgggglg
